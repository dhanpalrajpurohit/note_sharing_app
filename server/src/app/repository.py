...
class UserRepo:
    async def create(db: Session, instance: schemas.UserCreate):
        db_item = models.User(fname=instance.fname,lname=instance.lname,email=instance.email,password=Hasher.get_password_hash(instance.password))
        db.add(db_item)
        db.commit()
        db.refresh(db_item)
        return db_item
...
class NoteRepo:
    async def create(db: Session, instance: schemas.NoteCreate, user: models.User):
        db_item = models.Note(title=instance.title, description=instance.description, user=user)
        db.add(db_item)
        db.commit()
        db.refresh(db_item)
        return db_item
